    ...
    angular.module('FileManagerApp').controller('FileManagerCtrl', [
        ...
        function ($scope, $rootScope, $window, $translate, fileManagerConfig, Item, FileNavigator, ApiMiddleware) {
            ...
            $scope.smartClick = function (item) {
                ...
                if (item.isPDF()) {
                    console.log('pdf');
                    if ($scope.config.previewPDFInModal) {
            ...
            $scope.openImagePreview = function () {
                ...
                $('#imagepreview-target').show();
                $('#pdfpreview-target').hide();
            ...
            $scope.openPDFPreview = function () {
                var item = $scope.singleSelection();
                $('#imagepreview-target').hide();
                $('#pdfpreview-target').show();
                $('#pdfpreview-target iframe').attr('src', '');
                $scope.apiMiddleware.apiHandler.inprocess = true;
                $scope.modal('imagepreview', null, true);
                console.log($scope.apiMiddleware.getUrl(item));
                console.log(window.location);
                $scope.apiMiddleware.getPDFFile(item).then(function(data) {
                    $('#pdfpreview-target iframe').attr('src', 'src/js/pdf.js/web/viewer.html?file=' + data.result.path);
                    // $('#pdfpreview-target iframe').attr('src', 'js/pdf.js/web/viewer.html?file='+data.result.path);
                });
